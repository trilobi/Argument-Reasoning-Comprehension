...
class Model(object):
	...
	def __init__(self, max_len, word_vector, lstm_num_units, n_class):
		...
		with tf.variable_scope('dnn1_layer'):
			Context = add_layer(context, 512, 256, tf.nn.relu)		
		with tf.variable_scope('dnn2_layer'):
			Warrant0 = add_layer(warrant0, 256, 256, tf.nn.relu)
		with tf.variable_scope('dnn2_layer', reuse=True):
			Warrant1 = add_layer(warrant1, 256, 256, tf.nn.relu)
		W_m = tf.Variable(tf.truncated_normal([512, 1], stddev=0.1), name='W_m')
...
if __name__ == '__main__':
	...
	with tf.Session(config=config) as sess:
		...
		learning_rate = tf.train.exponential_decay(0.0002, global_step, decay_steps=len(train_labels)/batch_size, decay_rate=0.9)
