...
class Model(object):
	...
	def __init__(self, max_len, word_vector, lstm_num_units, n_class):
		...
		reason = self.get_last(reason_outputs, self.reason_len )
		...
		#------------------------------Target--------------------------------
		self.sim_warrant0 = tf.diag_part(tf.matmul(tf.matmul(reason, W_m), tf.transpose(warrant0, [1,0])))
		self.sim_warrant1 = tf.diag_part(tf.matmul(tf.matmul(reason, W_m), tf.transpose(warrant1, [1,0])))
